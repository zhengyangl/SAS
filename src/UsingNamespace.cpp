...
const char* const UsingNamespace::checkerName = "sas.CodeRules.UsingNamespace";
...
bool UsingNamespace::isDeclOK(const clang::NamedDecl* D, clang::ento::BugReporter& BR) const
{
   ...
   if (IsDisabled(D, BR, checkerName)) return true;                                          // Disabled by a comment
...
void UsingNamespace::reportBug(const char* bug, const clang::Decl* D, clang::ento::BugReporter& BR) const
{
   ...
   BR.EmitBasicReport(D, this, "using namespace in header files", "code rules", os.str(), DLoc);
}
