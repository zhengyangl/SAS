...
namespace sas
{
   ...
   void FiniteMathChecker::checkPreStmt(const clang::CallExpr* CE, clang::ento::CheckerContext& ctx) const
   {
      ...
      if (!II->isStr("isnan") && !II->isStr("isinf")) return;

      clang::ento::ExplodedNode* N = ctx.generateSink();
      if (!N) return;

      if (!BT) BT.reset(new clang::ento::BugType(this, "std::isnan / std::isinf does not work when fast-math is used. Please check the bits of the floating point number instead.", "SAS"));
