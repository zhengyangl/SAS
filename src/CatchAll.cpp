...
void CatchAll::checkASTCodeBody(const clang::Decl*& D, clang::ento::AnalysisManager& AM, clang::ento::BugReporter& BR) const
{
   ...
   BR.EmitBasicReport(D, this, "'catch(...)' in sources", "code rules", "using 'catch(...)' is forbidden", DLoc, s->getSourceRange());
}

const clang::Stmt* CatchAll::process(const clang::Stmt* S) const
{
   if (clang::CXXCatchStmt::classof(S) && checkCatchAll(static_cast<const clang::CXXCatchStmt*>(S))) return S;
