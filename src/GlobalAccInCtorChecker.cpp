...
namespace
{
   ...
   class SAICVisitor : public ConstStmtVisitor<SAICVisitor>
   {
    ...
    public:
      SAICVisitor(const Decl* const declWithIssue, BugReporter& br, std::unique_ptr<BugType>& BT, AnalysisManager& Mgr);
   ...
   void ProcessDeclRefExpr(const DeclRefExpr* const DRE, const Decl* const DeclWithIssue, BugReporter& BR, std::unique_ptr<BugType>& BT, AnalysisManager& Mgr);
   ...
   // Emit a report!
   void EmitReport(const DeclRefExpr* const DRE, const VarDecl* const varDecl, const Decl* const DeclWithIssue, BugReporter& BR, std::unique_ptr<BugType>& BT, AnalysisManager& Mgr);
} // end anonymous namespace

const char* const sas::GlobalAccInCtorChecker::checkerName = "sas.security.GlobalAccInCtor";

void sas::GlobalAccInCtorChecker::checkASTDecl(const CXXConstructorDecl* D, AnalysisManager& Mgr, BugReporter& BR) const
{
   if (!D) return; /// Invalid declaration
   /// `D` is a declaration of a constructor.
   if (!D->isThisDeclarationADefinition()) return; /// No definition attached to declaration
...
namespace
{
   SAICVisitor::SAICVisitor(const Decl* const declWithIssue, BugReporter& br, std::unique_ptr<BugType>& BT, AnalysisManager& Mgr) : BR(br), DeclWithIssue(declWithIssue), BT(BT), Mgr(Mgr) {}

   void SAICVisitor::VisitChildren(const Stmt* const S)
   {
      for (Stmt::const_child_iterator I = S->child_begin(), E = S->child_end(); I != E; ++I) {
         if (const Stmt* const child = *I) Visit(child);
      }
   }

   void SAICVisitor::VisitStmt(const Stmt* const S) { VisitChildren(S); }

   void SAICVisitor::VisitDeclRefExpr(const DeclRefExpr* const DRE) { ProcessDeclRefExpr(DRE, DeclWithIssue, BR, BT, Mgr); }

   void ProcessDeclRefExpr(const DeclRefExpr* const DRE, const Decl* const DeclWithIssue, BugReporter& BR, std::unique_ptr<BugType>& BT, AnalysisManager& Mgr)
   {
      if (!DRE) return; /// Invalid reference expression
      ...
      if (!varDecl) return; /// Not a variable declaration
      /// `varDecl` (that is `valueDecl`) is a variable delcaration.
      /// `DRE` refers to a variable.
      if (!varDecl->hasGlobalStorage()) return; // Isn't a global variable
      /// `varDecl` is a global variable.
      /// `DRE` refers to a global variable.
      if (HasConstantInitializer(varDecl)) return; // The variable has constant initializer
      /// Bingo!
   ...
   bool HasConstantInitializer(const VarDecl* const varDecl)
   {
      if (varDecl->hasInit()) {
         ...
         if (IsConstantInitializer(init, Ctx)) return true; /// The variable has constant initializer
      }
      return false;
   }
   ...
   void EmitReport(const DeclRefExpr* const DRE, const VarDecl* const varDecl, const Decl* const DeclWithIssue, BugReporter& BR, std::unique_ptr<BugType>& BT, AnalysisManager& Mgr)
   {
      ...
      if (!BT) BT.reset(new BugType("Global variable accessed in constructor", "Security"));
      const SourceManager& SM = BR.getSourceManager();
      AnalysisDeclContext* const analysisDeclContext = Mgr.getAnalysisDeclContext(DeclWithIssue);
      const LocationOrAnalysisDeclContext lac = analysisDeclContext;
      const PathDiagnosticLocation l = PathDiagnosticLocation::createBegin(DRE, SM, lac);
